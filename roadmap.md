...
- 2026-07-14: Made exact database membership authoritative over stale Authentik tenant-group claims and added the disabled-by-default permanent-account workflow: durable leased provisioning jobs, immutable OIDC/management UUID binding, one global enrollment sender per user, least-privilege tenant-group fencing, Team setup/retry/resend/disable states, and responsive QA. Production activation remains intentionally blocked until the dedicated Authentik service token, recovery email stage, and immutable UUID claim are configured and verified.
