...
- [x] **UI-045: Make one proof review resolve one item**
  ...
  - Status: complete and QA-covered. New proof supersedes the prior open review cycle atomically, resolves its evidence request, limits each submission to three photos, and stale/repeated review actions return a conflict instead of mutating the item again. Review controls now reject duplicate taps synchronously, keep unrelated rows usable, lock the active row/form with action-specific feedback, and ignore stale queue refreshes.
...
- [ ] **UI-048: Provision permanent accounts through Authentik**
  ...
  - Status: implementation and production identity wiring are complete behind `AUTHENTIK_PROVISIONING_ENABLED=false`. The immutable UUID scope is attached and requested in the deployed frontend, the recovery flow/stage exists, the dedicated least-privilege service identity and expiring token are active, its read-only API smoke test returns `200`, Coolify contains the required backend values, the API is healthy, and the disposable Authentik QA identity was removed. Production activation is intentionally blocked because the stored Brevo SMTP secret is incomplete and Authentik's direct send test returns `535 Authentication failed`; a valid relay credential and the signed-in phone/desktop smoke test are still required. One-time four-digit crew access remains the supported onboarding path meanwhile. Human administrator credentials must not be stored in the app.
...
- [ ] **UI-036: Loading and error states**
  ...
  - Status: in progress, 2026-07-15. Workspace, alert, settings, reports, and activity refreshes now reject same-tick duplicate taps, name the exact work in progress, disable conflicting controls, and keep prior report data when a refresh fails. Settings save/copy and tenant sign-out use the same synchronous action fencing. On phones, the closed report action menu leaves its trigger visibly labeled `Refreshing...`. Desktop/Pixel 7 regression coverage double-dispatches each action before React can disable it and proves one request. The fallback authentication shell and remaining non-mutating export/copy utilities still need final classification before closing `UI-036`.
