...
`876en.org` renders the public FRG/newsletter splash page. Its Login action points to `https://admin.876en.org/#/launch`; that route signs users in through Authentik and sends platform admins to admin, newsletter admins to the newsletter editor, single-platoon users to their platoon, or multi-platoon users to a chooser. A shared HttpOnly renewal cookie on `api.876en.org` avoids another Authentik trip when routing to a platoon subdomain. Authentik's tile dashboard is not part of the normal product flow. Tenant subdomains render the SaaS workspace by default; the old static lookup screen remains available at `/#/lookup` or `/lookup` during transition.
